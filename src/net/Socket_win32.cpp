...
std::string TcpSocket::recvUntil(const std::string& delimiter, int timeoutMs) {
    ...
    while (true) {
        ...
        int received = ::recv(sock, buffer, sizeof(buffer), MSG_PEEK);
        if (received == SOCKET_ERROR || received == 0) {
            if (received == 0) connected_ = false;
            break;
        }
...
std::string TcpSocket::recvExact(int bytes, int timeoutMs) {
    ...
    while (remaining > 0) {
        ...
        int received = ::recv(sock, buffer, toRead, 0);
        if (received == SOCKET_ERROR || received == 0) {
            if (received == 0) connected_ = false;
            break;
        }
...
void TcpSocket::setBlocking(bool blocking) {
    ...
    ioctlsocket(static_cast<SOCKET>(socket_), FIONBIO, &mode);
}
...
void TcpSocket::setTimeout(int sendMs, int recvMs) {
    ...
    setsockopt(sock, SOL_SOCKET, SO_SNDTIMEO, reinterpret_cast<char*>(&sendTimeout), sizeof(sendTimeout));
    setsockopt(sock, SOL_SOCKET, SO_RCVTIMEO, reinterpret_cast<char*>(&recvTimeout), sizeof(recvTimeout));
}
